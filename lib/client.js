		...
		function apply(ctx) {
			if (ctx.slots) {
				...
				ctx.slots.inject("settings.plugin.item", function () {
					return ctx.slots.register(
						{ name: "settings.plugin.item", key: "@yyfather/dsh-token-vault" },
						function () {
							return React.createElement("div", { style: { padding: "4px 0" } },
								React.createElement("div", { style: { fontSize: 14, fontWeight: 600 } }, "dsh-token-vault"),
								React.createElement("div", { style: { fontSize: 12, opacity: 0.75, marginTop: 4 } }, "凭证库：GitHub/npm 令牌安全保存在 DSH 凭证库，代理通过 vault_run 直接调用（gh/npm/npx/node/git），明文不出 Host。点击 设置 → 凭证库 管理令牌。")
							);
						}
					);
				});
